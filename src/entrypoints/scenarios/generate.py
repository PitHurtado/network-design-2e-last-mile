...
poetry run python -m src.entrypoints.scenarios.generate --regime normal --n 50
poetry run python -m src.entrypoints.scenarios.generate --all --n 50
"""

import argparse
import json

from src.constants import PATH_SHAPE_PARAMS, REGIMES, SEED_BASE
from src.scenarios.generate import ScenarioGenerator, generate_regime, write_manifest
...
def main() -> None:
    ...
    if not PATH_SHAPE_PARAMS.exists():
        parser.error(f"{PATH_SHAPE_PARAMS} not found — run src.entrypoints.scenarios.fit_params first")
