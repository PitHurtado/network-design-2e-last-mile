"""End-to-end check: do the generated scenarios feed the CA and Gurobi unchanged?
    ...
    poetry run python -m src.entrypoints.scenarios.verify_end_to_end --n 3
...
* the CA only writes cost keys for `demand > 0`, and `uncapacitated_saa_model`
...
def run_regime(regime: str, n_scenarios: int, max_run_time: float) -> dict:
    ...
    # The model records an objective without inspecting Status, so a time-limit
    # incumbent would otherwise be reported as if it were optimal.
