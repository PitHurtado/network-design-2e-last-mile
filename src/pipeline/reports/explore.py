...
Complements `src/pipeline/reports/scenarios.py` (which validates the aggregate structure
...
def build_report(regimes: list[str], output_path=None):
    ...
    layer_select = "".join(
        f'<option value="{layer}"{" selected" if layer == default_layer else ""}>{layer}</option>' for layer in layers
    )
...
"""
    ...
    if not drop_rel_diff:
        ...
        drop_insight += (
            ...
            "solo escala `stop`, no `drop` (`src/pipeline/generate.py`)."
        )
    else:
        drop_insight += (
