...
class UncapacitatedSAAModel:
    ...
    def __add_objective(self, facilities: Dict, scenarios: Dict[str, Scenario]) -> None:
        ...
        self.obj.cost_total = (1 / len(scenarios)) * (self.obj.cost_served_from_facilities + self.obj.cost_served_from_dc)
            ...
            for t in range(self.instance.periods):
                for k in scenario.pixels.keys():
                    ...
                    self.model.addConstr(
                        quicksum([self.vars.X[(i, k, t, n)] for i in facilities.keys()]) + quicksum([self.vars.W[(k, t, n)]])
                        >= 1,
