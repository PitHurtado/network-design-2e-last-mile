"""Interactive out-of-sample and VSS report for the flexibility policies."""
...
from src.visualization.components.labels import CASE_LABELS, FLEX_LABELS, REGIME_LABELS, SOLVE_STATUS_LABELS
from src.visualization.html import PLOTLY_CDN, PLOTLY_CFG
from src.visualization.html import fig_html as _fig_html
...
def _html(fig):
    return _fig_html(fig, config={**PLOTLY_CFG, "responsive": True})


@dataclass
class EvaluationPreviewData:
    """The evaluations finished so far, for a progress preview."""
...
class EvaluationReportData:
    """All 27 fixed-Y evaluations with their derived tables, built by `src.optimization.reports`."""
...
def _vss_figure(vss):
    ...
    values = [
        [
            vss.loc[
                (vss.regime == regime) & (vss.flexibility == flexibility) & (vss.baseline_solution == baseline),
                "vss_cost_reduction",
            ].iloc[0]
            for baseline in columns
        ]
        for regime, flexibility in rows
    ]
    fig = go.Figure(
        go.Heatmap(
            z=values,
            x=[CASE_LABELS[x] for x in columns],
            y=[f"{REGIME_LABELS[r]} · {FLEX_LABELS[f]}" for r, f in rows],
            colorscale=[[0, "#b94f4f"], [0.48, "#f4e9d8"], [0.5, "#ffffff"], [1, "#22855a"]],
            zmid=0,
            text=[[f"{value:,.0f}" for value in row] for row in values],
            texttemplate="%{text}",
            textfont={"size": 12},
            colorbar_title="Ahorro VSS",
            hovertemplate="%{y}<br>vs %{x}<br>VSS: %{z:,.2f}<extra></extra>",
        )
    )
    fig.update_layout(
        height=460, margin=dict(l=190, r=45, t=18, b=40), xaxis_side="top", paper_bgcolor="white", plot_bgcolor="white"
    )
    return fig
...
def _distribution_figure(scenarios):
    ...
    for group_index, ((regime, flexibility), _) in enumerate(groups):
        ...
        buttons.append(
            {
                "label": f"{REGIME_LABELS[regime]} · {FLEX_LABELS[flexibility]}",
                "method": "update",
                "args": [
                    {"visible": visible},
                    {"title": f"Distribución de costo total · {REGIME_LABELS[regime]} · {FLEX_LABELS[flexibility]}"},
                ],
            }
        )
    fig.update_layout(
        barmode="overlay",
        height=430,
        title=f"Distribución de costo total · {REGIME_LABELS[groups[0][0][0]]} · {FLEX_LABELS[groups[0][0][1]]}",
        xaxis_title="Costo anual por escenario",
        yaxis_title="Número de escenarios",
        plot_bgcolor="white",
        updatemenus=[{"buttons": buttons, "x": 0, "y": 1.18, "xanchor": "left", "yanchor": "top"}],
...
def _decision_comparison(installations):
    pivot = installations.pivot(
        index=["regime", "flexibility", "facility"], columns="solution_case", values="capacity"
    ).reset_index()
    rows = []
            ...
            rows.append(
                {
                    ...
                    "Política": FLEX_LABELS[flexibility],
                    "Solución comparada": CASE_LABELS[baseline],
                    "Capacidad Y base": group[baseline].sum(),
                    "Capacidad Y optimización": optimized_total,
                    "Satélites con Y distinta": int((group[baseline] != group["optimization"]).sum()),
                }
            )
...
def _pretty(frame):
    output = frame.copy()
    for source, target, labels in (
        ("regime", "Régimen", REGIME_LABELS),
        ("flexibility", "Política", FLEX_LABELS),
        ("solution_case", "Solución Y", CASE_LABELS),
        ("baseline_solution", "Solución base", CASE_LABELS),
    ):
        if source in output:
...
def _solution_status_table(means: pd.DataFrame) -> pd.DataFrame:
    ...
    status = means[
        [
            ...
            "source_status",
            "source_is_optimal",
            "source_optimality_gap",
            "source_actual_run_time",
        ]
    ].copy()
...
def render_preview(data: EvaluationPreviewData, output_path: Path) -> Path:
    """A clearly-labelled progress preview while the full VSS batch is running."""
    version, means, installations = data.version, data.means, data.installations
    ...
    decision_table = (
        _pretty(installations)
        .sort_values(["Régimen", "Política", "Solución Y", "facility"])
        .to_html(index=False, float_format=lambda x: f"{x:,.0f}")
    )
    means_table = labels.sort_values(["Régimen", "Política", "Solución Y"]).to_html(
...
<div id="top"><h1>Vista parcial de evaluaciones binarias</h1><p>Versión {version} · {data.n_evaluations} de 27 evaluaciones disponibles</p></div>
...
def render(data: EvaluationReportData, output_path: Path) -> Path:
    version, means, scenarios, installations = data.version, data.means, data.scenarios, data.installations
    percentiles, vss, theoretical = data.percentiles, data.vss, data.theoretical
    ptable = (
        _pretty(percentiles)
        .sort_values(["Régimen", "Política", "Solución Y"])
        .to_html(index=False, float_format=lambda x: f"{x:,.0f}")
    )
    vtable = (
        _pretty(vss)
        .sort_values(["Régimen", "Política", "Solución base"])
    ...
    winners = percentiles.loc[
        percentiles.groupby(["regime", "flexibility"])["mean_total"].idxmin(),
        ["regime", "flexibility", "solution_case", "mean_total", "p95_total"],
    ]
