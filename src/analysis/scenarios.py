...
from src.constants import N_PERIODS, PATH_SHAPE_PARAMS, REGIME_TARGETS, RESULTS_DIR
from src.data.demand_panel import load_panel
from src.scenarios.generate import ScenarioGenerator, load_generated
...
def load_all(regimes: list[str]) -> dict:
    ...
    for regime in regimes:
        from src.constants import scenario_dir
...
def build_report(regimes: list[str], output_path=None):
    ...
    agg_insight = (
        f"Los tres regímenes caen donde deben: "
        ...
        f"bastante menor que la dispersión del histórico ({(historical.quantile(0.9) - historical.quantile(0.1)) / historical.mean() * 100:.0f}%), "
